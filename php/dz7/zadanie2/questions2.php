    ...
    foreach ($correctAnswers as $index => $answers) {
        if (isset($_POST['question' . $index])) {
            ...
            if (array_diff($answers, $userAnswers) === [] && count($userAnswers) === count($answers)) {
                $score += 3; // 3 балла за правильный ответ
            }
        }
    }
    $_SESSION['score2'] = $score2; // Сохраняем количество правильных ответов
    header('Location: questions3.php'); // Переход на третью страницу
    exit();
}
?>
